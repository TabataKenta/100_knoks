import os
from dotenv import load_dotenv
import google.generativeai as genai
import csv
import time

# 1. 環境設定の読み込み
load_dotenv()
api_key = os.getenv("GOOGLE_API_KEY")

if not api_key:
    print("APIキーが見つかりません。.envを確認してください。")
    exit()

# 2. APIキーを設定
genai.configure(api_key=api_key)

# 3. モデルの準備
model = genai.GenerativeModel("models/gemini-2.5-flash")

# 4. CSV読み込み
questions_data = []
with open('global_facts.csv', encoding='utf-8') as f:
    reader = csv.reader(f)
    for row in reader:
        if len(row) < 6:
            continue
        
        # 元のデータを取り出す
        question_text = row[0]
        options_list = [row[1], row[2], row[3], row[4]] # [Aの中身, Bの中身, Cの中身, Dの中身]
        correct_char = row[5]
        
        # 1. 元の正解がリストの何番目か調べる (A=0, B=1, C=2, D=3)
        char_map = {'A': 0, 'B': 1, 'C': 2, 'D': 3}
        correct_index = char_map[correct_char]
        
        # 2. 「正解の文言」と「ハズレの文言リスト」に分ける
        correct_text = options_list[correct_index]
        wrong_texts = [text for i, text in enumerate(options_list) if i != correct_index]
        
        # 3. ハズレをA,B,Cに詰め込み、正解を強制的にDにする
        # 新しい並び: A=ハズレ1, B=ハズレ2, C=ハズレ3, D=正解
        new_options_str = f"A. {wrong_texts[0]}\nB. {wrong_texts[1]}\nC. {wrong_texts[2]}\nD. {correct_text}"
        
        new_answer = "D"
        
        questions_data.append({
            "question": row[0],
            "options": new_options_str, # 新しい選択肢を使う
            "answer": new_answer
        })

# 正解数カウント用
correct_count = 0
total_count = 0

print(f"-------採点開始（全{len(questions_data)}問）-------")

for i, item in enumerate(questions_data):
    prompt = f"""
以下は多肢選択式の問題です。正解の記号（A, B, C, D）のみを出力してください。
余計な解説は不要です。

問題: {item['question']}
{item['options']}

答え:
""" 
    # 6. 生成実行
    try:
        response = model.generate_content(prompt)
        
        # AIの答え（余白削除）
        prediction = response.text.strip()
            
        # 正解（CSVの答え）
        ground_truth = item['answer'].strip()
        
        # 判定
        # AIが空文字を返す場合のエラー対策を追加しておくと安全です
        if prediction:
            is_correct = prediction.startswith(ground_truth)
        else:
            is_correct = False
        
        if is_correct:
            correct_count += 1
            result_mark = "⭕️"
        else:
            result_mark = "❌"
        
        print(f"問{i+1}: {result_mark} (AI: {prediction} / 正解: {ground_truth})")
        total_count += 1
        
        # 無料枠対策
        time.sleep(5)
        
    except Exception as e:
        print(f"エラーが発生しました: {e}")
        time.sleep(5)

# 4. 最終結果の表示
if total_count > 0:
    accuracy = correct_count / total_count
    print(f"\n最終結果: 正解率 {accuracy:.2%} ({correct_count}/{total_count})")
